...
严格模式是针对当前启用4个策略的选择进行执行交易。如果启用4个策略，运行时4个策略同时发出做多或做空，才会执行交易。是一个非常保守的模式。尚未测试正确性，因为很难触发这个条件。

同理，如果启用3个策略，也需要3个策略都发出相同信号才执行交易逻辑。

### 1.3 多数模式 不受阈值限制

多数模式是超过半数策略的信号才会触发交易逻辑。启用4个策略，如果有3个策略是做多信号，1个策略是做空信号，则执行买入逻辑。
