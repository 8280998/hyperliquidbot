...
### 运行了2个月，成绩很失败。从2026年1月1日号更新了策略，地址不变。  重新编写了整个交易流程和策略，截止2026年2月4号止，目前演示地址已涨到约1300。从跌倒最少时只有450左右，到现在回本并赚取30%左右。感觉已经成熟，但还需继续测试。
...
## 2025-11-18 增加一个副信号功能。主要功能为双保险，如果启用后，必须主信号和副信号都发出相同策略发进行交易。避免单个K线产生的信号的过于灵敏或滞后。（尚未更新到github代码中）
...
###  2025-11-09 开放源代码
###  2025-11-10 周期数据添加5m和30m，删除4h技术线
###  2025-11-16 测试了几个策略，于11月14号改回使用1天K线，权重使用趋势跟踪型，信号阈值为0.55.到16号截止基本回归到正常波动。使用分钟K线和小时K线时发出的策略信号更容易变动，导致交易方向频繁变化。
###  2025-11-18 发现一个严重问题，如果获取币安api数据出错，但达到止盈止损条件会导致代码这个逻辑。
###  2025-11-18 增加一个副信号功能。主要功能为双保险，如果启用后，必须主信号和副信号都发出相同策略发进行交易。避免单个K线产生的信号的过于灵敏或滞后。（尚未更新到github代码中），当前实盘已启用副信号功能（主信号1d+副信号30m，趋势跟踪型）。
### 2026-02-05 2026年初到现在，全部重新编写，但保留技术指标为策略生成信号。目前已平稳运行且盈利状态不错。从1000测试起步跌到最低450，到目前回升到1400左右。并且周胜率保持在80%以上。
<img width="2670" height="1240" alt="QQ_1770269284010" src="https://github.com/user-attachments/assets/4638d660-143e-4b2e-b8a7-508b48e0d2bf" />
